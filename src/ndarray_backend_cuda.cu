...
CudaDims CudaTwoDim(size_t x, size_t y) {
  ...
  dim.block = dim3(B, B, 1);
  dim.grid = dim3((x + B - 1) / B, (y + B - 1) / B, 1);
  return dim;
}
...
__device__ size_t getSparseIndexOffset(const CudaVec &sparseStrides,
                                       const CudaVec &shape, size_t gid) {
  size_t ns = 1, ret = 0;
  for (int i = shape.size - 1; i >= 0; --i) {
    int idx = gid / ns % shape.data[i];
    ret += sparseStrides.data[i] * idx;
    ns *= shape.data[i];
  }
  return ret;
}
...
__global__ void CompactKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  /// BEGIN YOUR SOLUTION
  out[gid] = a[offset + getSparseIndexOffset(strides, shape, gid)];
...
__global__ void EwiseSetitemKernel(const scalar_t *a, scalar_t *out,
  ...
  /// BEGIN YOUR SOLUTION
  out[offset + getSparseIndexOffset(strides, shape, gid)] = a[gid];
...
__global__ void ScalarSetitemKernel(scalar_t a, scalar_t *out, size_t size,
                                    CudaVec shape, CudaVec strides,
                                    size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;

  /// BEGIN YOUR SOLUTION
  out[offset + getSparseIndexOffset(strides, shape, gid)] = a;
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out,
                                size_t reduce_size) {
  size_t dim = blockDim.x * blockIdx.x + threadIdx.x;
  size_t start_idx = dim * reduce_size;
  scalar_t val = a[start_idx++];
  for (int i = 1; i < reduce_size; ++i) {
    val = val > a[start_idx] ? val : a[start_idx];
    ++start_idx;
  }
  out[dim] = val;
}
